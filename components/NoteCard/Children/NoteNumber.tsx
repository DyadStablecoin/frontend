      ...
      <div className="w-full mt-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {data.map((item: NoteNumberDataColumnModel, index: number) =>
            item.isComponent ? (
              <div
                className={`w-full ${data.length % 2 !== 0 && index === data.length - 1 ? "cols-span-1 md:col-span-2" : "cols-span-1"} col-start-1 row-start-1 md:col-start-auto md:row-start-auto`}
              >
                {item.value}
              </div>
            ) : (
              <div
                key={index}
                className={`py-2.5 ${data.length % 2 !== 0 && index === data.length - 1 ? "cols-span-1 md:col-span-2" : "cols-span-1"}`}
              >
                <div
